...
fit_stan_joint_model_exact_floor <- function(
  ...
  model_prefix = Sys.getenv(
    "GLW_MODEL_PREFIX",
    unset = "stan_joint_interval_dmr_exact_floor"
  )
) {
  ...
  stan_file <- file.path(project_root, "04_Code", "Stan", "glw_joint_interval_dmr_exact_floor.stan")
  ...
  cat("Saved exact-floor Stan fit:", fit_rds, "\n")
  cat("Saved exact-floor Stan summary:", summary_csv, "\n")
  cat("Saved exact-floor CmdStan CSV output files:", draws_dir, "\n")
  invisible(fit)
}

if (sys.nframe() == 0) {
  fit_stan_joint_model_exact_floor()
}
