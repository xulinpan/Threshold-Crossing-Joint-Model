data {
  ...
  array[N_visit] int<lower=1, upper=N_pat> id_visit;
  array[N_quad] int<lower=1, upper=N_pat> id_quad;
  ...
  vector<lower=0>[N_visit] t_visit;
  vector<lower=0>[N_quad] t_quad;
  vector<lower=0>[N_quad] w_quad;
...
transformed parameters {
  matrix[N_pat, 2] b;
  vector[N_pat] u_visit;

  for (i in 1:N_pat) {
    b[i, 1] = tau_b[1] * z_b[1, i];
    b[i, 2] = tau_b[2] * z_b[2, i];
    u_visit[i] = sigma_u * z_u[i];
  }
}
...
model {
  ...
  delta0 ~ normal(log(2), 1);
  delta_mrd ~ normal(0, 0.5);
  delta_time ~ normal(0, 0.5);
  ...
  // Observed monitoring visits. Exclude deterministic baseline visits when
  // constructing t_visit outside Stan.
  for (r in 1:N_visit) {
    int i = id_visit[r];
    real lt = log1p(t_visit[r]);
    real mu_visit = beta0 + beta_time * lt + beta_time2 * square(lt) +
      b[i, 1] + b[i, 2] * lt;
    real log_lambda = delta0 + delta_mrd * (mu_visit + 4.5) +
      delta_time * lt + u_visit[i];
    target += log_lambda;
  }

  // Quadrature approximation to cumulative visit intensity.
  for (q in 1:N_quad) {
    int i = id_quad[q];
    real lt = log1p(t_quad[q]);
    real mu_quad = beta0 + beta_time * lt + beta_time2 * square(lt) +
      b[i, 1] + b[i, 2] * lt;
    real log_lambda = delta0 + delta_mrd * (mu_quad + 4.5) +
      delta_time * lt + u_visit[i];
    target += -w_quad[q] * exp(log_lambda);
  }
}
...
generated quantities {
  ...
  vector[N_pat] log_lik_visit_patient = rep_vector(0, N_pat);
  ...
  for (r in 1:N_visit) {
    ...
    real log_lambda = delta0 + delta_mrd * (mu_visit + 4.5) +
      delta_time * lt + u_visit[i];
    log_lik_visit_patient[i] += log_lambda;
  }
  ...
  for (q in 1:N_quad) {
    ...
    real log_lambda = delta0 + delta_mrd * (mu_quad + 4.5) +
      delta_time * lt + u_visit[i];
    log_lik_visit_patient[i] += -w_quad[q] * exp(log_lambda);
  }
}
